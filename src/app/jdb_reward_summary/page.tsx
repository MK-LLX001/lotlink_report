...
import { exportJdbReward, fetchJdbTax5Rows, type JdbRow, type JdbTaxRow } from "@/lib/exportjdb";
...
// Columns ຕາມ SQL: ງວດ | ລາງວັນ Sokxay | ໂຊກຊ້ອນໂຊກ | ຄ່າທຳນຽມ | ອາກອນ5% | ລວມທັງໝົດ
const COLS: { key: keyof JdbRow | "ອາກອນ5%"; label: string }[] = [
  { key: "ງວດ",                   label: "ງວດ" },
  { key: "ຈຳນວນລາງວັນ Sokxay",    label: "ຈຳນວນລາງວັນ Sokxay" },
  { key: "ໂຊກຊ້ອນໂຊກ",            label: "ໂຊກຊ້ອນໂຊກ" },
  { key: "ຄ່າທຳນຽມ",               label: "ຄ່າທຳນຽມ" },
  { key: "ອາກອນ5%",               label: "ອາກອນ 5%" },
  { key: "ລວມທັງໝົດ",               label: "ລວມທັງໝົດ" },
];
...
export default function JdbRewardSummaryPage() {
  ...
  const fetchData = async (from: string, to: string) => {
    ...
    try {
      ...
      // Query 2: ອາກອນ 5% individual items (SPLUS_PRICE_TAX, BANK_CR)
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await exportJdbReward(rows, taxItems, appliedFrom, appliedTo);
    } catch (e) {
  ...
  // ຮວມ dataRows + taxItems ໂດຍ index (ຄືກັນກັບ BCEL pattern)
  ...
  // ອາກອນ5% total = sum ຂອງ taxItems ທັງໝົດ
  const tax5Total = useMemo(
    () => taxItems.reduce((s, t) => s + t.BANK_CR, 0),
    [taxItems],
  );
  ...
  const grandTotal = useMemo(() => {
    ...
    return parseN(totalRow["ລວມທັງໝົດ"]);
  }, [totalRow]);
  ...
  const TH  = "px-2 py-2 text-center font-bold text-slate-700 whitespace-nowrap bg-orange-50 border border-black text-[11px]";
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr>
                    ...
                    <th className={TH}>ງວດ</th>
                    <th className={TH}>ຈຳນວນລາງວັນ Sokxay</th>
                    <th className={TH}>ໂຊກຊ້ອນໂຊກ</th>
                    <th className={TH}>ຄ່າທຳນຽມ</th>
                    <th className={TH}>ອາກອນ 5%</th>
                    <th className={TH}>ລວມທັງໝົດ</th>
                  </tr>
                </thead>
                <tbody>
                  {dataRows.map(({ row, tx }, i) => (
                    <tr key={i} className="hover:bg-orange-50">
                      ...
                      <td className={TD + " font-bold"}>{row ? fmtVal(row["ລວມທັງໝົດ"]) : ""}</td>
                    </tr>
                  ))}
                  ...
                  {totalRow && (
                    <tr className="total-row bg-gray-200 font-bold">
                      ...
                      <td className={TD + " bg-gray-200 font-bold"}>{fmtVal(totalRow["ຈຳນວນລາງວັນ Sokxay"])}</td>
                      ...
                      <td className={TD + " bg-gray-200 font-bold"}>{fmtVal(totalRow["ລວມທັງໝົດ"])}</td>
                    </tr>
                  )}

                  {/* ລວມຈ່າຍທັງໝົດ grand total */}
